...
<Page title="Dialog">
	...
	>
		<div
			class="fixed inset-0 | bg-raisin-05/90 backdrop-filter backdrop-blur-1px"
			use:overlay
			transition:fade
		/>
		<div
			class="w-120 p-8 z-10 | grid gap-6 | bg-raisin-10 rounded-l"
			transition:scale={{ start: 1.1 }}
			use:content
		>
			<div class="grid gap-3">
				<DialogTitle class="text-2xl text-white font-semibold">
			...
			{#if isShowingFocusableChildren}
				<div class="flex gap-3">
					<button class="button-small" on:click={close}> Cancel </button>
					<button class="button-small" on:click={close}> Delete </button>
					{#if isShowingRef}
						<button class="button-small" on:click={close} id="ref" bind:this={initialFocus}>
							Initial Focus
						</button>
					{/if}
				</div>
	...
	<Dialog class="fixed inset-0 | grid place-content-center" bind:open={showing} {initialFocus}>
		<DialogOverlay class="fixed inset-0 | bg-raisin-05/90 backdrop-filter backdrop-blur-1px" />
		<DialogContent class="w-120 py-8 z-10 | grid gap-6 | bg-raisin-10 rounded-l" let:close>
			<div class="grid gap-3">
				<DialogTitle class="px-8 | text-2xl text-white font-semibold">
					Do you want to delete your account?
				</DialogTitle>
				<div class="w-full h-0.5 | bg-raisin-15" aria-hidden />
			...
			{#if isShowingFocusableChildren}
				<div class="px-8 | flex gap-3">
					...
					<button class="button-small" on:click={close}> Delete </button>
					{#if isShowingRef}
						<button class="button-small" on:click={close} bind:this={initialFocus}>
							Initial Focus
						</button>
					{/if}
				</div>
